...
[$user->id, $tweet->id])) { $is_liked = true; }
@endphp
  ...
  <main class="max-w-[80rem] px-2 my-5 mx-auto">
    <div class="flex">
      ...
      <div class="w-full max-w-[50rem]">
        <div class="flex gap-3 border-b border-white/50 pb-5">
          <img class="w-[3rem] h-[3rem] object-cover self-start rounded-full bg-white" src="{{$user->profile_picture}}" />

          <div class="w-full">
            <p class="px-2 rounded-md focus:outline-none w-full text-lg mb-4">{{$tweet->content}}</p>

            <form class="flex gap-10 tweet_menu">
              @csrf
              <button hx-swap="innerHTML" hx-get="/reply?id={{$tweet->id}}" hx-target="#reply"
                title="Reply" class="cursor-pointer flex items-center gap-1">
                ...
                <span class="text-white/50 text-sm">{{$replies_count}}</span>
              </button>
              ...
              <button
                ...
                <span id="likesCounts-{{$tweet->id}}" class="text-white/50 text-sm">{{$likes_count}}</span>
              </button>
            </form>
          </div>
        </div>
...
<script hx-indicator="#spinner" hx-get="/replies?c=0&tweet_id={{$tweet->id}}" hx-trigger="revealed" hx-target="#tweets">
