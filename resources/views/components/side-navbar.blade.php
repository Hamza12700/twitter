...
<dialog class="border w-full max-w-[40rem] fixed top-1 border-zinc-400 md:mt-20 bg-black text-white mx-auto rounded-md" id="post_tweet_dialog">
  <form hx-on::after-request="window.location.reload()" hx-post="/tweet" class="p-3">
    @csrf
    <div class="flex">
    ...
    <button class="bg-white ml-auto text-black block px-4 font-semibold text-lg py-2
      rounded-lg cursor-pointer" type="submit">Tweet</button>
  </form>
</dialog>
...
<aside class="w-fit mr-3 md:mr-5 border-neutral-500 border-r pr-2 md:pr-10">
  <div class="sticky top-9">
    ...
    <button onclick="open_tweet_dialog()" class="bg-white text-black font-bold w-full rounded-full py-2 text-xl
      cursor-pointer" title="Tweet">Post</button>
  </div>
</aside>

<script>
function open_tweet_dialog() {
  document.getElementById("post_tweet_dialog").showModal();
}
</script>
